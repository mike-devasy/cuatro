...
Parallax.Each = class {
  constructor(parent) {
    ...
    this.offset = 0
    this.value = 0
    this.smooth = parent.dataset.flsParallaxSmooth
      ? Number(parent.dataset.flsParallaxSmooth)
      : 15
    this.setEvents()
  }
  setEvents() {
    ...
    this.elements.forEach((el) => {
      const parameters = {
        ...
        direction: el.dataset.flsParallaxDirection
          ...
          : "-1",
        coefficient: el.dataset.flsParallaxCoefficient
          ? Number(el.dataset.flsParallaxCoefficient)
          : 5,
        additionalProperties: el.dataset.flsParallaxProperties
          ? el.dataset.flsParallaxProperties
          : "",
      }
...
export function initParallax() {
  const parallaxParents = document.querySelectorAll(
    "[data-fls-parallax-parent]",
  )
  if (parallaxParents.length) {
    new Parallax(parallaxParents)
  }
}